...
Using this expression for identifiability analysis, we find that each of the parameters $V_3^{max}$, $K_3^{fdp}$ and $K_3^{pep}$ have two different close form expressions owing to the presence of a square root term in their solutions. These distinct expressions are denoted by (1) and (2) following the respective parameter names throughout the rest of the document. 

We believe this problem of computational tractability will occur in other complex kinetic rate law formulations as well. Example?

\subsection{Maximum reaction rates are more identifiable than enzyme binding constants:}
...
\begin{figure}[!tbhp]
	\centering{\includegraphics[width=1.0\textwidth,height=0.3\textheight, keepaspectratio]{C:/Users/shyam/Documents/Courses/CHE1125Project/Results/ident/figure2/v1max_k1cat_all_experiments_jan01}}
...
\end{figure}
 ...
 This is further illustrated in Figure \ref{fig:figure2} where the different experiments in each combination identifying both $V_1^{max}$ and $k_1^{cat}$ are shown.
