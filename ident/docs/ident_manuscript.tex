...
We found that the ability of a combination of experiments to identify a particular parameter widely varies depending the the position of the flux in the metabolic network with respect to the inputs and the outputs of the network. In Figure \ref{fig:figure1} we show the number of combinations of experiments that are capable of identifying each parameter in each flux. In the left panels (Figure \ref{fig:figure1}a-c), the data is show when noise-free data is used for identification. The identifiability information for parameters using data with additive noise is shown on the right panel (Figure \ref{fig:figure1}d-f). 
