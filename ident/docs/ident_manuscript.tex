...
If all intracellular metabolite concentrations can be measured over a time course, a nonlinear programming problem can be formulated to estimate the enzyme kinetic parameters ($\theta$) in Equation (\ref{eq:kinstoich}), based on the measured data. The minimization of least square error between the measured ($x^*$) and modeled ($x$) concentrations is used as an objective function (Equation \ref{eq:lsqopt}a) for the optimization problem (Equation \ref{eq:lsqopt}), with fixed upper ($\theta_u$) and lower ($\theta_l$) bounds for the parameter values (Equation \ref{eq:lsqopt}b). The difference between the data and the model estimate for each output and at each time point in the objective function is weighted by the variance in the experimental data $\sigma_{kl}^*$ for that corresponding variable and time point.
...
Algorithms for structural identification of parameters have 
Algorithms have been extensively developed for establishing structural identifiability of dynamic models of biological systems \parencite{Audoly2001}(IEEE Trans paper from 2007). Most of these algorithms use methods based on differential algebra \parencite{Ljung1994}. The underlying motivation for these methods is the need to change the non-convex parameter estimation problem (Equation \ref{eq:lsqopt}) to a convex problem due to inherent difficulties in obtaining global optimal solutions for non-convex problems \parencite{Bellu2007}. However, due to the use of computer algebra systems to solve the identifiability problem, these methods are not scalable to large systems like a kinetic model of a large metabolic network .

Recently proposed methods address the issue of scalability by using a profile-likelihood-based approach to establish structural identifiability \parencite{Raue2009a}. Profile likelihood-based methods offer an advantage over the differential algebra methods due to their ability to not only structurally identify parameters, but also help with experimental design through practical identifiability of parameters.

In contrast to differential algebra-based methods, these methods still depend on solving a non-convex nonlinear least squares problem (Equation \ref{eq:lsqopt}) to get likelihood estimates of parameters. Hence, these methods can still be considered to be un-scalable for structurally identifying parameters in large metabolic networks. Nonetheless, 



Apart from issues with their scalability, profile likelihood-based methods also depend on the availability of actual measured or simulated dynamic data to establish structural identifiability.


Although, improvements in measurement technologies can enable one to obtain high frequency dynamic data to address the need for time course data required by these methods, 


The profile likelihood-based methods and the differential algebra-based methods are heavily dependent on the availability of 

 for all the output variables of the system and scale poorly with increase in size of the modeled system. 


However, computational burden due to poor scalability can be partly addressed with the current increases in computational power. 



, no scalable methodologies exist to utilize the abundantly available steady state data for parameter estimation and identifiability for large metabolic networks. 
...
Here, we demonstrate the use of our computer algebra-based methodology to establish practical identifiability of parameters for fluxes $v_1$, $v_2$ and $v_3$ in the small model of gluconeogenesis described in Section \ref{sec:small-model}. For the purposes of our identifiability methodology, we assume that relevant steady state metabolite concentrations and fluxes can be measured.
...
{v_1}^{(k)} = V_{1}^{max}\frac{ac^{(k)}}{ac^{(k)}+K_{1}^{ac}} && \forall k=\{1, 2\}
\end{align}
...
\begin{subequations}\label{eq:v1_par}
	...
	\end{align}
\end{subequations}
In Equation (\ref{eq:v1_par}), the denominator of the right hand side expression is used to test the identifiability of parameters $V_1^{max}$ (Equation \ref{eq:v1_par}a) and $K_1^{ac}$ (Equation \ref{eq:v1_par}b) for different available experimental data set combinations. Since the enzyme binding constant ($K_1^{ac}$) and maximum reaction rate ($V_1^{max}$) cannot be negative, we can further reduce the criteria for identifiability for both these parameters by saying that the evaluated expressions should be non-negative. 
...
In Figure \ref{fig:figure1} we show the number of combinations of experiments that are capable of identifying each parameter in each flux. In the left panels (Figure \ref{fig:figure1}a-c), the data is shown when noise-free data is used for identification. The identifiability information for parameters using data with additive noise is shown on the right panel (Figure \ref{fig:figure1}d-f). As seen in these figures, irrespective of the presence or absence of noise in the data, the number of combinations that can identify each parameter varies widely, even for parameters within the same flux. 

Since both fluxes $v_1$ and $v_2$ are represented by the same enzyme kinetic rate law, we compare the number of data combinations that are capable of identifying the different parameters used to model these fluxes. We observe that the number of data combinations identifying the maximum reaction rates ($V_i^{max}$) for both $v_1$ (Figure \ref{fig:figure1}a and d) and $v_2$ (Figure \ref{fig:figure1}b and e) is always greater for than the number of data combinations that identify the enzyme binding ($K_i$) constants for both of these fluxes, irrespective of the presence of noise in the experimental data. 

Although, only about 20\% of the 4896 different data combinations can identify parameters in $v_1$, the difference between the number of data combinations being able to identify the different parameters in $v_1$ is less than 2\% (Figure \ref{fig:figure1}a and d). However, the same could not be said for the number of data combinations identifying parameters in $v_2$. In contrast to the high number of data combinations identifying $V_2^{max}$ (89.5\%), less than 2\% of the combinations are able to identify $K_2^{pep}$ (Figure \ref{fig:figure1}b and e).

The trend noted earlier for $v_1$ and $v_2$ with regards to the number of data combinations identifying the reaction rate parameters versus the number of data sets identifying the corresponding enzyme binding constants in these fluxes is also seen for $v_3$, i.e., the percentage of data combinations identifying $V_3^{max} (1)$ and $V_3^{max}(2)$ is greater than the percentage of combinations that can identify $K_3^{fdp}(1)$ and $K_3^{fdp}(2)$. 

We found that the ability of a combination of experiments to identify a particular parameter widely varies depending the the position of the flux in the metabolic network with respect to the inputs and the outputs of the network. 

As seen from Figure \ref{fig:figure1}, 

Both in the presence as well as absence of noise, the identifiability of parameters in $v_1$ is guaranteed by only about 20\% of the available data combinations (Figure \ref{fig:figure1}a, d). As seen in Section \ref{sec:example}, the identifiability expressions for both parameters in $v_1$ are dependent on changes in the both the uptake flux $v_1$ as well as the acetate concentration $ac$ in both the experiments used for identification. If either of these values cannot be distinguished between the two experiments whose data is used for identification, then the corresponding combination of experiments can be guaranteed to result in practical non-identifiability of the parameters in $v_1$. This is further illustrated in Figure below where the different experiments in each combination are shown.
...
\end{figure}
\begin{figure}[!tbhp]
	\centering{\includegraphics[width=1.0\textwidth,height=0.3\textheight]{C:/Users/shyam/Documents/Courses/CHE1125Project/Results/ident/figure2/k1cat_no_noise_experiments_jan03}}
