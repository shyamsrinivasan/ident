...
\begin{document}
	...
	The use of metabolic engineering spans a wide variety of applications. Some notable examples include the design of microorganisms for the biosynthesis of commodity and specialty chemicals \parencite{Andreozzi2016}, engineering mammalian cells as therapeutic targets for cures to some ailments affecting humans \parencite{DiFilippo2016,Apaolaza2017}, and changing the constituents of the human gut microbial community to cure related diseases \parencite{Zerfab2018}. These applications require us to understand the numerous complex interaction, their roles in cell function, and sometimes even the mechanisms behind these interactions \parencite{Pushalkar2018}. Computational models offer a systematic way to integrate available experimental data, and to study and understand these interactions through mathematical representations of the biological systems in which these interactions occur \parencite{Bordbar2014a,Saa2017}. They are also used to predict changes in cell function based on changes in the type and nature of the modeled interactions \parencite{Andreozzi2016}, or aid in the identification of therapeutic targets for drug discovery and development \parencite{Bordbar2015,Chandrasekaran2017}
	
	Constraint-based models (CBMs) of metabolism are one such category of models that are used to improve our understanding of metabolism by representing it as a stoichiometric network of reactions that operate under a pseudo-steady state assumption \parencite{Bordbar2014a}. The ability of CBMs to shine light on the nonintuitive interactions that govern cellular metabolism is leveraged to engineer and asses the impact of designs that alter the ability of a cell to grow, or produce a desired metabolite \parencite{Maia2016}. However, in CBMs, since metabolism is assumed to operate under a pseudo steady state, the metabolite concentrations within the metabolic network are assumed to be constant, and changes in metabolite concentrations are not modeled. Furthermore, since CBMs represent metabolism using only the stoichiometry of its constituent reactions, they do not account for the various non-catalytic regulatory interactions that are also responsible of metabolic function. These shortcomings prevent CBMs from being used to fully understand the steady state as well as the dynamic characteristics of metabolic networks. 
	
	Recently, the focus has shifted towards understanding the implications of metabolite concentrations and associated regulatory interactions on the steady state and the dynamic characteristics of metabolism \parencite{Saa2017}. Kinetic models accounting for the thermodynamic and regulatory interactions that underly metabolic networks in addition to its stoichiometry are used for this purpose \parencite{Link2014}. These models can not only help us learn about previously unknown characteristics of metabolism \parencite{Kotte2014}, but can also improve predictions about the impact of engineering design perturbations on metabolism, and propose alternative designs to achieve various goals \parencite{Andreozzi2016,Khodayari2016}. 
	
	Kinetic models differ from CBMs in their use of heavily parameterized mechanistic enzyme kinetic rate laws to model enzyme catalyzed fluxes within a metabolic network. These parameters represent various aspects of the enzyme kinetic rate laws \parencite{Srinivasan2015,Saa2017}. Hence, the use of kinetic models requires information on all the enzyme kinetic rate laws that will be used to model all the fluxes within a metabolic network, as well as numerical values for all the parameters used in these rate laws. Analyzing the ability of a metabolic network to exhibit dynamic characteristics like multiple steady states and oscillations, irrespective of the structure of the network, is one example where kinetic rate laws and parameter values might play a crucial role \parencite{Vital-Lopez2006,Srinivasan2017}. 
	
	The development of these enzyme kinetic rate laws is based on in vitro observations of enzyme activity. Accordingly, authors have questioned their relevance for gleaning information on the dynamics of metabolism under in vivo conditions, as opposed to in vitro conditions \parencite{Heijnen2013}. This problem is further compounded by the fact that typical parameter values used in kinetic models of metabolism are estimated in vitro, and may not be applicable under in vivo conditions \parencite{Heijnen2005,Smallbone2007}. Moreover, estimation of kinetic parameters based on experimental in vivo data becomes computationally expensive for larger models, and any estimated in vivo parameter estimates are associated with large uncertainties \parencite{Link2014}. Using in vitro, or unreliable in vivo parameter estimates to study the steady state and dynamic responses of metabolic networks, under in vivo conditions, to different perturbations reduces confidence in the model predicted behaviour \parencite{Andreozzi2016a}. Consequently, this hampers the use of these models to gain insight into metabolic network function \parencite{Chakrabarti2013a}, and the increase in prediction uncertainty becomes an obstacle for using the predicted responses as a basis for designing the metabolic networks to achieve any of the aforementioned goals of metabolic engineering and design \parencite{Saa2017}.  
	
	So, the use of heavily parameterized enzyme kinetic rate laws not only differentiates kinetic models from CBMs of metabolism, but is also a major hurdle to the widespread use and adoption of kinetic models \parencite{Berthoumieux2013,Vasilakou2016}. In order to address this problem, authors have proposed two different strategies. In one scenario, authors have looked at the development and use of global optimization algorithms for parameter estimation \parencite{Moles2003}. In another case, in vitro kinetic rate law descriptions are abandoned in favour of approximate and new empirical enzyme kinetic models whose parameters can not only be estimated in a relatively easier fashion, but also lead to kinetic models that generate information that has a higher correlation with experimental data that is observed in vivo \parencite{Heijnen2005,Smallbone2007}. 
	
	However, one of persistent problems is the lingering uncertainty in the parameter estimates and the subsequent predictions that are obtained from these methods. Authors have proposed to alleviate this parameter uncertainty either by using a Monte Carlo approach to kinetic modeling of metabolism \parencite{Srinivasan2015}, or by quantifying this uncertainty in parameter estimates using different methods \parencite{Vanlier2013}. Approaches to parameter estimation from a Bayesian perspective have also been proposed to estimate parameters and quantify the uncertainty in these estimates \parencite{Saa2016}.	
	
	In spite of all these developments, model parameter identifiability, a necessary condition to estimate unique kinetic parameter values from experimental data, is often overlooked \parencite{Ljung1994,Berthoumieux2013}. Briefly, it concerns with the ability to distinguish between any two different parameter values based on differences in the inputs to, and consequently the responses exhibited by the cell. Structural identifiability refers to the dependence of the ability to estimate unique parameter values on model structure, due to inherent dependencies between two or more parameters in the model \parencite{Berthoumieux2013}. Conversely, when the dependence is on the informativeness of the data used for estimation, and not on the structure of the model, it is referred to as practical identifiability \parencite{Ljung1994}. 
	
	Significant work has been done towards the development of methods for structural identification of parameters in kinetic models of metabolism \parencite{Ljung1994,Nikerel2009,Berthoumieux2013,Raue2014}\textcolor{red}{(paper from Rudiyanto Gunawan on model discrimination and sensitivity analysis)}. However, the focus on methods for practical identification of parameters in kinetic models of metabolism and its use for experimental design is fairly recent and limited \parencite{Gadkar2005a,Vanlier2014a,Raue2014}. For instance some of these methods are limited by their applicability to approximate kinetic models only \parencite{Nikerel2009,Berthoumieux2013}, and some perform identifiability within a dynamic context \parencite{Gadkar2005a,Nikerel2009,Vanlier2014a}. Some other methods also suffer from their inability to be applied to kinetic models of large metabolic networks due to the associated computational burden \parencite{Gadkar2005a,Raue2014}\textcolor{red}{(Banga method using FIM for D-optimal design, ??)}. 	
		
	In this paper, we propose a scalable methodology that uses available steady state fluxomics, metabolomics and proteomics data to practically identify parameters for each individual reaction in kinetic models of metabolism. We demonstrate how the computer algebra-based method that we have developed can also facilitate the design of experiments that are minimal and informative to generate data required to estimate unique parameter values for all reaction fluxes in a metabolic network. In doing so, we not only propose specific types of and nature of perturbations that will provide the most useful data for parameter estimation, but also test the identifiability of different enzyme kinetic rate laws that are typically used to model fluxes in metbaolic networks. 
	
	 For the purposes of this method we assume that all intracellular metabolite concentrations and fluxes can be measured. We illustrate our methodology to identify parameters and design experiments for estimating parameters in a small metabolic network model of glucoeneogenesis in \textit{Escherichia coli} \parencite{Kotte2014, Srinivasan2017}.  %We also demonstrate the scalability of the proposed methodology to facilitate experimental design by applying it to a relatively larger metabolic network of the human red blood cell hepatocyte.
...
%Bayesian approach to identification and experimental design \parencite{Vanlier2012}.
	...
	\subsection{Definition: Structural and Practical parameter identifiability of kinetic models of metabolism}\label{sec:kinetic_model}
