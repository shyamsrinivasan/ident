...
\begin{document}
	...
	\textbf{Outline:}
	\begin{itemize}
		\item Need for models in studying biology. State various applications e.g., for bio-based production, for medicine (gut microbiome interactions, stem cell engineering, studying various diseases like cancer)
		\item Constraint-based models as a way to peek into the black box of biology without too many parameters. They however do not account for a lot of factors
		\item Kinetics models as an improvement over constraint-based models
		\item Advances in kinetic modeling of metabolism (Reviews?)
		\item Parameter estimation in kinetic models as part of these advances
	\end{itemize}
	...
	The use of metabolic engineering spans a wide variety of applications, ranging from the design of microorganisms for the biosynthesis of commodity and specialty chemicals \parencite{Andreozzi2016} to engineering mammalian cells as therapeutic targets for cures to various ailments that affect humans \parencite{Bordbar2015,DiFilippo2016,Apaolaza2017}. These applications require us to understand the numerous complex interaction, their roles in cell function, and sometimes even the mechanisms behind these interactions. Computational models offer an avenue to study and understand these interactions through simplified representations of these interactions. They are also used to predict changes in cell function based on changes in the type and nature of the modeled interactions, or finding therapeutic targets to cure various ailments \textcolor{red}{(cite from old draft)}.	
	
	Constraint-based models (CBMs) of metabolism are one such category of models that are used to improve our understanding of the flow of flux through metabolism by representing it as a stoichiometric network of reactions that operate under a pseudo-steady state assumption. The ability of CBMs to shine light on the nonintuitive interactions that govern cellular metabolism is leveraged to predict the effect of different metabolic engineering design decisions on the ability of a cell to grow, or produce a desired metabolite (Bordbar, Monk, et al. 2014; Maia, M. Rocha, and I. Rocha 2016). These advantages, combined with minimal number of parameters that are required to make CBMs functional, have made them useful to get an initial understanding of metabolism and its relationship to cellular function for various aforementioned applications \textcolor{red}{(CBM use references? cMCS, MCS may be?)}. 
	
	However, since metabolism is assumed to operate under a pseudo steady state, the metabolite concentrations within the metabolic network are assumed to be constant, and CBMs of metabolism do not model changes in metabolite concentrations. Furthermore, since CBMs represent metabolism using only the stoichiometry of its constituent reactions, they do not account for the various non-catalytic regulatory interactions that are also responsible of metabolic function. These shortcomings prevent CBMs from being used to fully understand the steady state as well as the dynamic characteristics of metabolic networks. 
	
	Recently, the focus has shifted towards understanding the importance of the dynamics of metabolism in determining cell function \textcolor{red}{(recent kinetic model review about their utility)}. Consequently, it has been shown that using kinetic models that can represent the dynamics of the network can not only help us learn about previously unknown characteristics of metabolism \textcolor{red}{(red blood cell-both old and new, Maranas bifurcation paper)}, but can also improve predictions about the impact of engineering design perturbations on metabolism \textcolor{red}{(ecoli k457, k-optForce, Maranas kinetic model paper)}. \parencite{Bordbar2015, Andreozzi2016}. These kinetic models of metabolism, in contrast to their constraint-based counterparts, account for the thermodynamic and regulatory interactions that underly metabolic networks in addition to its stoichiometry \parencite{Andreozzi2016a}. 
	
	Kinetic models differ from CBMs is their use of mechanistic and empirical enzyme kinetic rate laws to model enzyme catalyzed fluxes within a metabolic network. The other aspect is the fact that kinetic models are heavily parameterized. These parameters represent various aspects of the enzyme kinetic rate laws \textcolor{red}{(kinetic model reviews, our review and a more recent one)}. Hence, the use of kinetic models requires information on all the enzyme kinetic rate laws that will be used to model all the fluxes within a metabolic network, as well as numerical values for all the parameters used in these rate laws. These aspects not only differentiate kinetic models from CBMs of metabolism, but are also two of the major hurdles to their widespread use and adoption. 
	
	Firstly, the development of these enzyme kinetic rate laws is based on observations of enzyme activity that are made in vitro. Accordingly, several authors have questioned their relevance for gleaning information on the dynamics of metabolism under in vivo conditions, as opposed to in vitro conditions \textcolor{red}{(Heijnen and Klamt)}. This problem is further compounded by the fact that typical parameter values used in kinetic models of metabolism are estimated in vitro, and may not be applicable under in vivo conditions. Any available in vivo parameter estimates are associated with large uncertainties. Analyzing the ability of a metabolic network to exhibit dynamic characteristics like multiple steady states and oscillations, irrespective of the structure of the network, is one example where parameter values might play a crucial role \parencite{Srinivasan2015, Vital-Lopez2006,Srinivasan2017}. Hence, the ability to study the steady state and dynamic responses of metabolic networks, under in vivo conditions, to different perturbations is compromised. The use of in vitro, or unreliable in vivo parameter estimates reduces confidence in the model predicted behaviour \parencite{Andreozzi2016a}. Consequently, the reduction in confidence hampers the use of these models to gain insight into metabolic network function \parencite{Chakrabarti2013a, Bordbar2015}. The corresponding increase in uncertainty in model predicted responses becomes an obstacle for using the predicted responses as a basis for designing the metabolic networks to achieve any of the aforementioned goals of metabolic engineering and design.  
	
	Efforts to improve kinetic models have been dedicated to the development of newer enzyme kinetic models that have a higher correlation with what is observed in vivo. Authors have taken different approaches to address this problem. Some have focused on the development and improvement of parameter estimation methods based on optimization principles for obtaining true in vivo parameter values based on available in vivo experimental data without changing the underlying kinetic rate laws used in the model \textcolor{red}{Isabel Rocha papers on optimization methods, Japanese group that did parameter estimation using a HPC cluster}. Others have sought to completely abandon in vitro kinetic rate law descriptions in favour of approximate kinetic rate law models whose parameters are more informative and can be estimated in a relatively easier fashion \textcolor{red}{(Heijnen and other approximate kinetic modeling formats papers, Kotte 2010)}. 

	
	Despite these efforts, one of persistent problems is the lingering uncertainty in the parameter estimates and the subsequent predictions that are obtained from these methods. Some authors have proposed to alleviate this parameter uncertainty by using a Monte Carlo approach to obtaining kinetic models of metabolism \textcolor{red}{(ensemble modeling and ORACLE)}. Here, experimental steady state data is used as a reference point and kinetic models are built around this steady state value. Although a some have sought to quantify this uncertainty in parameter estimation through methods based on sensitivity analysis \textcolor{red}{(Nathan van Riel and Heijnen papers on uncertainty analysis)}, others have sought to approach parameter estimation from a Bayesian perspective \textcolor{red}{(Lars Nielsen papers)}.
	
	These new kinetic models are either formulated on the basis of approximate enzyme kinetic rate laws to alleviate 
	
	
	
	In the absence of any in vivo parameter estimates, the norm is to either use available in vitro enzyme kinetic parameters, or use different parameter estimation techniques to determine parameter values based on either in vitro or in vivo data. So, the ability of these enzyme rate laws, and kinetic models built with these rate laws to study, understand and predict in vivo metabolic network function is limited \textcolor{red}{Heijnen}.
	
	
	
	 but have also focused on methods to obtain parameter values that are applicable under in vivo conditions, as opposed to in vitro enzyme kinetic parameters. Research in the former area has resulted in the proposal of numerous approximate kinetic models of metabolism
	
	
	the prediction of dynamic responses of metabolic networks to different perturbations using kinetic models of metabolism is dependent on numerous enzyme kinetic parameters.  that are used to represent the changes in both concentrations and fluxes 
	
	the numerical values of the enzyme kinetic parameters ($\theta$ in Equation \ref{eq:kinstoich}) \parencite{Andreozzi2016a}. Analyzing the ability of a metabolic network to exhibit dynamic characteristics like multiple steady states and oscillations, irrespective of the structure of the network, is one example where parameter values might play a crucial role \parencite{Srinivasan2015, Vital-Lopez2006}. 
...
>>>>>>> 716f16b43c50e6c1d8ea567b372bdd233ad0d691
...
>>>>>>> 716f16b43c50e6c1d8ea567b372bdd233ad0d691
	...
	%As mentioned in section \ref{sec:intro}, practical identifiability is a necessary condition to estimate true values of both $V_1^{max}$ and $K_1^{ac}$ from experimental data. 	
	...
	\begin{figure}[!tbhp]
		\centering{\includegraphics[width=1.0\textwidth,height=1.0\textheight,keepaspectratio]{figures/figure4/v1_v1max_v2_v3_1_data_utility}}
	...
	\begin{figure}[!tbhp]
		\centering{\includegraphics[width=1.0\textwidth,height=1.0\textheight,keepaspectratio]{figures/figure1/v1_k1cat_v1max_v2_v3_1_2_ident}}
	...
	\begin{figure}[!tbhp]
		\centering{\includegraphics[width=1.0\textwidth,height=0.3\textheight, keepaspectratio]{figures/figure1/v5_1_2_ident}}
		\caption{identifiability for flux v5 with 2 roots for Kefdp}
	\end{figure}
	
	\begin{figure}[!tbhp]
		\centering{\includegraphics[width=1.0\textwidth,height=0.3\textheight, keepaspectratio]{figures/figure4/v5_1_2_data_utility}}
	...
	\begin{figure}[!tbhp]
		\centering{\includegraphics[width=1.0\textwidth,height=0.3\textheight, keepaspectratio]{figures/figure4/v1_k1cat_v3_2_data_utility}}
	...
	\begin{figure}[!tbhp]
		...
		\caption{missing caption}%\label{fig:figure2}
	\end{figure} 
	...
	\begin{figure}[!tbhp]
		\centering{\includegraphics[width=1.0\textwidth,height=0.3\textheight, keepaspectratio]{figures/figure1/v3_1_2_ident}}
		\caption{missing caption}%\label{fig:figure2}
	\end{figure} 
	...
	\section{Old Figures and Figure Captions}	
