    ...
    if any(tfr)
        ...
        switch(model.rxns{tfr})
            ...
            case 'NADH16'
                ...
                flux(tfr) = Knadh16/3600*(Emq8-Emnad_nadh+Z/2*log(prod(mc(sbid))/prod(mc(prid)))-2*pmf);
            case 'CYTBD'
...
%                 rUbiOxi_t = K4c*mu + K4*(1/(1+(Km4/pO2)^E3))*(1/(1+(pO2/Km4i)^E3i));
                UbiOxi_t = 0.5;
                UbiOxi_red = UbiOxi_t/(mc(q8)/mc(q8h2)*exp((pmf-EmUbiOxi+Emq8)/Z)+1);
                flux(tfr) = KUbiOxi*UbiOxi_red*mc(q8h2)/(1+(KmUbiOxi/pO2)^hill_UbiOxi);
            case 'SUCDi'
                % succ + q8 <---> fum + q8h2
