...
%**************************************************************************
function [pact] = singlepromoteractivity_v5(Protein,srate,brate,RS,...
                  GeneRules,bindaff,igene,defparval)

%Assuming all parameters are passed in as vectors as opposed to as matrices
%srate is included now
%Others will follow!
%srate is passed along as a row vector (srates for each gene)
...
%                         pactnr(irule) = srate(igene,tf)*1;
                        ...
                        %Store this srate - store srates only for
                        %inhibitory TFs
...
%                         psrate(irule) = srate(1,tf);
            ...
            elseif iprot > 1 && iprot < nprots 
                ...
                [pactnr,pactdr,psrate_prot] = promoter_act(nextlogic,RS,iprot,pactnr,pactdr,psrate_prot);
                %old_tf = [old_tf,tf];
                old_tf(:,iprot) = tf;
                %prev_tf = tf;
                nextlogic = terms{iprot}{2};                
        ...
        if pactnr(irule) == 1            
            ...
            pactnr(irule) = (sum(psrate_prot(psrate_prot>0))/(length(psrate_prot(psrate_prot>0))))*pactnr(irule);
        end
        irule = irule + 1;        
    end
...
function [pactnr,pactdr,psrate] = promoter_act(logic,RS,kprot,pactnr,pactdr,psrate)
%     effect = RS(1,tf);
        ...
        switch RS(1,tf)
            case 1
                ...
                elseif prev_effect == -1 %previous_effect = -1
                    if pactnr(irule) == 1
                        %pactnr(irule) = pactnr(irule)*srate(1,tf)*bindaff(tfcount);
                        pactnr(irule) = pactnr(irule)*(srate(1,tf)+psrate(kprot-1))*bindaff(tfcount)/2;
                    else
                        pactnr(irule) = pactnr(irule) + srate(1,tf)*bindaff(tfcount);
                    end
                end                
