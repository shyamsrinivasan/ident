function [kfwd,kbkw,flux,vflux] = fluxATPS4r(model,pvec,mc,flux)
if nargin<4
...
Katps = 220;
Kc = 2.5;
k1 = pvec.kcat_bkw(vatps4r);
k2 = pvec.kcat_fwd(vatps4r);

% proton gradient dependent kcat
...
% mchc = 0.1*mc(he)*exp((log(prod(mc(prid)))-log(prod(mc(sbid)))-log(model.Keq(vatps4r)))/4);
...
% delGr = 0.008314*298*(log(prod(mc(prid).*1e-3))-log(prod(mc(sbid).*1e-3))-log(model.Keq(vatps4r))-...
%         4*log(mc(hc)/mc(he)));
