...
function [x_opt,opt_pid,new_opt_p,fval] =...
        flux2(opts,xss2,fss2,plist,old_opt_p)
if nargin<5
    old_opt_p = [];
end
    
% flux 3 
p_id = cellfun(@(x)strcmpi(plist,x),{'K2pep','V2max','K2fdp'},'UniformOutput',false);
...
lb = [1e-6;1e-3;1e-6];
ub = [20;2000;20];
[x_opt,fval,~,~,opts] = runoptim_flux(opts,@obj_flux2_CAS,lb,ub,p,optim_p);
