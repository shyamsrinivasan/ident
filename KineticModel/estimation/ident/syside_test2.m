...
[xdyn_fun,ode,x,p,ident_c,p_useless,acetate] = RK4integrator_cas(@kotteCASident,1);

xinit = no_noise_sol(4).xss;
xexp_data = no_noise_sol(1).xdyn(:,2:end);

x_sym =...
xdyn_fun(no_noise_sol(4).xss,repmat(p,1,3000),.1,repmat(opts.odep(14:16)',1,3000),.1);
