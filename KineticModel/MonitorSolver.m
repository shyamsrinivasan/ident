...
function [newdata] = MonitorSolver(call,T,Y,YQ,YS,data)
newdata = [];
...
if call == 0
    ...
    data.first = true;
    data.initialized = false;%since post = false => graph windows are uninitialized
    newdata = data;
    return;
    ...
    if data.first %if first call
        ...
        if ~isempty(Y)
            data.Nvar = length(Y);
            data.select = 1:data.Nvar;
            data.y = zeros(data.Nvar,data.update);
            data.sol = true;
        end
        if isempty(YS)
            data.sensi = false;
...
if data.post && (n == data.update || call == 2)
    ...
    if call == 2
        %Print final values
        if data.stats
            graph_final(hfg,npg,data.stats);
        end
        if data.cntr
            print_text(n,data.cntr,t,nst,nfe,nni,netf,nctf);
        end
