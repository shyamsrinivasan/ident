function [flux,vflux] = TKineticsADMAT(model,pvec,mc,Vex)
...
%     end
    ...
    if any(sbid) && any(prid)
        ...
        vflux(Vex(irxn)) = scale_fluxADMAT(nr_flux/dr_flux);
    else
        vflux(Vex(irxn)) = 0;
    end
...
%     end   
    vflux(Vex(irxn)) = scale_fluxADMAT(vflux(Vex(irxn)));
    flux(Vex(irxn)) = Vmax(Vex(irxn))*vflux(Vex(irxn));
end
flux = flux(Vex);
vflux = vflux(Vex);
