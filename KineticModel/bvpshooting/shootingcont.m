...
status = 1;
while status
    [tftest,~,flag1] = integrateshooting(@HoltODE,ti,tf,yi,opts);
    if flag1<0
        tf = (tf+tfnew)/2;
    elseif flag1>0
        status = 0;
...
while status
    [yic,yfc,tfnew,delyic,delyfc,flag] = execshooting(@HoltODE,ti,tf,yi,[],yterm,delyi,[],yiunkwn,yfknwn,eps,opts);
