function mkinDiagon_kry_NSSORtrial
...
% return solution to Px = v or x = inv(P)v
% solve for d in (f(x+eps*d)-f(x))/eps-v = 0
...
for i=1:neq
  yold = y(i);
  F = @(x)Jsysfn(x,i);
  J = @(d)((F(y(i)+eps*d)-F(y(i)))/eps-v(i));
  d = fsolve(J,0);    
  d(i) = (1-omega)*yold + omega*d;
end

new_data = []; % data was not modified
flag = 0;      % success

function fy = Jsysfn(y, i,data)
    fy = y^2 - i^2;
